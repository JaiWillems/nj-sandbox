...
float previousPitchAngle = 0;
float previousRollAngle = 0;
...
void loop()
{
  ...
  // gyroAngleX = gyroAngleX + filterGyroX * dt;
  // gyroAngleY = gyroAngleY + filterGyroY * dt;
  // float angleRoll = 0.98 * gyroAngleX + 0.02 * accelRoll;
  // float anglePitch = 0.98 * gyroAngleY + 0.02 * accelPitch;
  float alpha = 0.98;
  float anglePitch = complementaryFilter(previousPitchAngle, accelPitch, filterGyroY, dt, alpha);
  float angleRoll = complementaryFilter(previousRollAngle, accelRoll, filterGyroX, dt, alpha);

  // Serial.print(accelRoll);
  // Serial.print(",");
  // Serial.print(filterGyroX);
  // Serial.print(",");
